...
AppController.prototype.createCall_ = function() {
    ...
    //var element = document.getElementById(UI_CONSTANTS.privacyLinks);
...
var Call = function(params) {
  ...
  this.connectionHub_ = params.connectionHub;
  this.connectionHub_.on("onReceiveMessage", this.onRecvSignalingChannelMessage_.bind(this));
  this.channel_ = new SignalingChannel(params.wssUrl, params.wssPostUrl, params.connectionHub);
  this.pcClient_ = null;
  this.localStream_ = null;
...
Call.prototype.connectToRoom_ = function(roomId) {
  this.params_.roomId = roomId;
    var channelPromise = this.channel_.open()
        .catch(function (error) {
            this.onError_("Sinalling open error: " + error.message);
  ...
  Promise.all([channelPromise, joinPromise]).then(function() {
    ...
    this.onError_("register error: " + error.message);
  }.bind(this));
};
...
Call.prototype.maybeGetIceServers_ = function() {
  ...
  if (shouldRequestIceServers) {
    ...
    iceServerPromise = requestIceServers(requestUrl, this.params_.iceServerTransports, this.params_.authToken).then(function(iceServers) {
      ...
      if (this.onstatusmessage) {
        var subject = encodeURIComponent("ICE servers not working");
...
Call.prototype.joinRoom_ = function() {
  return new Promise(function(resolve, reject) {
    ...
    sendAsyncUrlRequest("POST", path, this.params_.authToken).then(function(response) {
...
Call.prototype.sendSignalingMessage_ = function(message) {
  ...
  if (this.params_.isInitiator) {
    ...
    this.connectionHub_.invoke("Send", this.params_.roomId, this.params_.clientId, msgString);
  }
};
...
var InfoBox = function(infoDiv, call, versionInfo) {
    this.infoDiv_ = document.getElementById("info-div") ;//infoDiv;
...
; var SignalingChannel = function (wssUrl, wssPostUrl, connectionHub, startedPromise) {
  this.wssUrl_ = wssUrl;
  ...
  this.onerror = null;
    this.connectionHub_ = connectionHub;

};
SignalingChannel.prototype.open = function() {
  trace("Opening signaling channel.");
    return new Promise(function (resolve, reject) {
        this.connectionHub_.on("onReceiveMessage", function (event) {
            trace("signalr->C: " + event.data);
        ...
        async function start() {
            await this.connectionHub_.start();
            if (this.connectionHub_.state === signalR.HubConnectionState.Connected) {
                trace("connected");
            }
            else {
                trace("Failed to connect. Trying again...");
                setTimeout(() => start(), 5000)
            }
        }
        let p = start.bind(this);
        p();
       resolve();
  }.bind(this));
};
...
SignalingChannel.prototype.register = function (roomId, clientId) {
    ...
    this.connectionHub_.invoke("Register", this.roomId_, this.clientId_);
    this.registered_ = true;
    trace("Signaling channel registered.");
};
SignalingChannel.prototype.close = function(async) {
   if (this.connectionHub_) {
       this.connectionHub_.stop();
  }
...
SignalingChannel.prototype.send = function (message) {
    ...
    trace("C->send: " + message);
    this.connectionHub_.invoke("Send", this.roomId_, this.clientId_, JSON.stringify(message));
...
function sendAsyncUrlRequest(method, url, authToken, body) {
    return sendUrlRequest(method, url, authToken,true, body);
}
  function sendUrlRequest(method, url, authToken,async, body) {
  return new Promise(function(resolve, reject) {
    var xhr;
...
function requestIceServers(iceServerRequestUrl, iceTransports, authToken) {
  return new Promise(function(resolve, reject) {
    sendAsyncUrlRequest("POST", iceServerRequestUrl, authToken).then(function(response) {
      ...
      resolve(iceServerRequestResponse.ice_servers);
    }).catch(function(error) {
